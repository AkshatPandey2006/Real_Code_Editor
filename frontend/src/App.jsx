...
const useRoom = () => {
  ...
  // 2. Initialize PeerJS (Video) - FIXED (No Destroy to prevent crash)
  useEffect(() => {
    const p = new Peer();
    p.on("open", (id) => {
      setMyPeer(p);
    ...
    // We intentionally do NOT destroy the peer here, because React Strict Mode
    // in development runs this twice and kills the connection immediately.
  }, []);
  ...
  // 5. Connect to New Users (Call Logic) - FIXED (Added Null Checks)
  useEffect(() => {
    if (!socket || !myPeer || !myStream) return;

    users.forEach((user) => {
      // Validate that user has a PeerID and it's not us
      if (user.peerId && user.peerId !== myPeer.id && !peers[user.peerId]) {
        try {
          ...
          call.on("stream", (userVideoStream) => {
            setRemoteStreams((prev) => {
              ...
              return [
                ...prev,
                { peerId: user.peerId, stream: userVideoStream, userName: user.userName },
              ];
            });
          });
...
const App = () => {
  ...
  const resize = useCallback(
    (e) => {
      if (isResizing && e.clientX > 200 && e.clientX < 500)
      ...
      <div className="landing-container">
        ...
        <nav className="navbar">
          <div className="nav-content">
            <div className="logo">
              <span className="logo-glitch">{`</>`}</span> CollabCode
            </div>
            <div className="nav-right">
              <a
                href="https://github.com/AkshatPandey2006/Real_Code_Editor"
                target="_blank"
          ...
          <div className="hero-content-wrapper">
            ...
            <div className="hero-left fade-in-up">
              ...
              <p className="hero-sub">
                ...
                programming practice. Spin up instant coding rooms with a
                powerful, VS Code-like editor.
              </p>
              <div className="hero-cta">
                <button
                  className="primary-btn large"
                  onClick={() =>
                    document.getElementById("join-box").scrollIntoView({
                      behavior: "smooth",
                    })
                  }
                >
            ...
            >
              ...
              <div className="glass-panel join-panel">
                ...
                <div className="input-group">
                  <label>
                    AGENDA <span className="optional">(OPTIONAL)</span>
                  </label>
                  <input
                    value={agenda}
        ...
        <footer className="footer">
          <div className="footer-content">
            ...
            <div className="footer-links">
              ...
              <a href="https://www.linkedin.com/in/akshatpandey2006/">
                LinkedIn
              </a>
            </div>
          </div>
    ...
    <div className="app-container">
      ...
      <div className="sidebar" style={{ width: sidebarWidth }}>
        <div className="sidebar-header">
          ...
          <h2 className="agenda-title" title={agenda}>
            {agenda || "Untitled Session"}
          </h2>
        </div>
        ...
        <div className="video-grid">
          {myStream && (
            <VideoPlayer stream={myStream} muted={true} label="You" />
          )}
          {remoteStreams.map((s) => (
            <VideoPlayer
              key={s.peerId}
              stream={s.stream}
              label={
                users.find((u) => u.peerId === s.peerId)?.userName || "User"
              }
            />
          ))}
        </div>
        ...
        <div className="users-section">
          <div className="section-title">
            <Icons.Users /> ONLINE ({users.length})
          </div>
        ...
        <button className="disconnect-btn" onClick={leaveRoom}>
          Disconnect
        </button>
      </div>
